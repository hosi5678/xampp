...
function create_members_input_form({
	...
	// console.log('mode:'+mode);
		...
		// 画面の更新
	 const parent_tag=childNodeClear(parent_tag_str+'_params');
	 const form=document.createElement('form');
								
			// form.name='form_'+parent_tag_str+'_'+mode;

	 const title=childNodeClear(parent_tag_str+'_title');
	 ...
	 p.innerText='メンバーの新規登録';
			...
			let koumoku=document.createElement('div');
				...
				koumoku.classList.add('members_seimei');
				// ここのタイミングでappendしないとidが認識されない。(存在しない)
				parent_tag.appendChild(koumoku); 

				for(let i=0;i<label.length;i++){
					if(label[i]=='姓'||label[i]=='名'){
						create_container({
							parent_tag_str:parent_tag_str+'_row1',
							class_str:'container',
							id:parent_tag_str+'_box'+i,
						});
					}
